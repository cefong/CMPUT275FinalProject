    ...
    while(1) {
        ...
        if(!stat) {
            ...
            if(unit->is_active) {
                // get end time for bullet
                timestamp_end_b = chVTGetSystemTime();
                if(unit[2].x <= unit[0].x + 15 && unit[2].x >= unit[0].x - 15 
                    && timestamp_end_b - timestamp_start_b >= TIME_MS2I(time_delay_bullet)) {
                    // fire bullet when in line with player and if time delay has passed
                    unit[2].is_fire = true;
                    // restart bullet time
                ...
                for(int i = 0; i < PLAY_NUM_BULLET; i++) {
                    // iterate through bullets
                    if (ammo[i].active && ammo[i].player 
                    && ammo[i].x >= unit[2].x - 15 && ammo[i].x <= unit[2].x + 15
                    && ammo[i].y <= unit[2].y + 50 && ammo[i].y >= unit[2].y + 10) {
                        // if bullet is active and within range, bot changes direction
                        if(is_left == 1) is_left = 0;
                        else is_left = 1;
                    }
                ...
                // constrain x position of bot
                unit[2].x = constrain(unit[2].x, size*SCALE, WIDTH - size*SCALE);
                if(unit[2].x <= size*SCALE && !is_left) {
                    is_left = 1;
                }
                else if(unit[2].x >= WIDTH - size*SCALE && is_left) {
                    is_left = 0;
                }
                // constrain y position of bot
                if (unit[2].y >= HEIGHT - 95) {
                    // if alien reaches bottom, respawn and remove a player life
                    unit[2].is_active = false;
                    unit[0].lives--;
                }
            }
        }
