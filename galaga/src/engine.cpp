...
static void processTouchScreen(int16_t *screen_x, int16_t *screen_y) {
    ...
    PARAMETERS:
        screen_x: x position of the touch on the screen
        screen_y: y position of the touc on the screen
    */
...
static void endScreen(int currentScore, int highScore, int mode){
    ...
    */
    ...
    while(start != 1) {
        ...
        // update high score if player beat the current high score
        ...
        // return to main menu when player presses joystick button
        if(butt_trig == 1) {
            start = 1;
            if(is_highscore) {
                switch(mode) {
...
static void show_selection() {
    /*
    Toggle between PLAY and HIGH SCORE options, and highlight the current selected options.
    */
...
static void singleplayer() {
    ...
    */
    ...
    while(start == 0){
        ...
        // if player has bullets left, get start time
        if(unit[0].bullets > 0) {
            time_start_bul = chVTGetSystemTime();
        }
        // if player has no bullets left, start timing 2 sec for reload
        else {
            time_end_bul = chVTGetSystemTime();
            // if reloading is finished, reset to 10 bullets
            if(time_end_bul - time_start_bul >= TIME_I2MS(RELOADING)) {
                unit[0].bullets = 10;
            }
        }

        // if player pressed button and they have bullets
        ...
        // if player touches main menu button
...
void engine() {
    ...
    */
    if(start == 1) {
        // if we are on main menu screen
        menu();
    }
    else if(start == 0) {
        // if player chose play
        singleplayer();
    }
    else if(start == 3) {
